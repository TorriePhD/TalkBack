...
function sampleLogFrequencyEnergy(
  ...
  // Emphasize the speech-heavy band while still spanning the full analyser spectrum.
  const minBin = 1;
  const maxBin = totalBins - 1;
  const logMin = Math.log(minBin + 1);
  const logMax = Math.log(maxBin + 1);
  const mapped = Math.exp(logMin + (logMax - logMin) * normalizedPosition) - 1;

  const centerBin = clamp(Math.round(mapped), minBin, maxBin);
  const spread = 2 + Math.round(normalizedPosition * 4);
  let total = 0;
  let weightTotal = 0;

  for (let offset = -spread; offset <= spread; offset += 1) {
    const sampleBin = centerBin + offset;
    if (sampleBin < minBin || sampleBin > maxBin) {
      continue;
    }

    const distance = Math.abs(offset) / Math.max(1, spread);
    const weight = 1 - distance * 0.72;
    total += frequencyData[sampleBin] * weight;
    weightTotal += weight;
  }

  return weightTotal > 0 ? total / weightTotal / 255 : 0;
}
      ...
      if (!lastFrameAtRef.current || now - lastFrameAtRef.current > TARGET_FRAME_MS) {
        ...
        if (analyser && frequencyData && smoothed && isPlaying && !prefersReducedMotion) {
          ...
          for (let i = 0; i < segmentCount; i += 1) {
            // Logarithmic bin mapping spreads audible speech activity around more of the circle.
            const normalizedPosition = i / Math.max(1, segmentCount - 1);
            const normalized = sampleLogFrequencyEnergy(frequencyData, normalizedPosition);
