create table if not exists public.round_listen_usage (
...
create index if not exists idx_round_listen_usage_user
on public.round_listen_usage (user_id, updated_at desc);

create or replace function public.get_round_listen_state(listen_round_id uuid)
returns table (
  round_id uuid,
  user_id uuid,
...
begin
  if current_user_id is null then
    raise exception 'You must be logged in to view listen usage.';
  end if;
  ...
  if round_row.recipient_id <> current_user_id then
    raise exception 'Only the recipient can view listen usage for this round.';
  end if;
  ...
  select
    ...
    current_user_id,
    listen_count_value,
    greatest(listen_count_value - free_limit_value, 0),
    free_limit_value,
    5,
...
begin
  ...
  insert into public.round_listen_usage as rlu (
    round_id,
    user_id,
    listen_count
  )
...
alter table public.round_listen_usage enable row level security;

revoke all on public.round_listen_usage from authenticated;
revoke all on public.round_listen_usage from anon;
revoke all on public.round_listen_usage from public;

grant all on public.round_listen_usage to service_role;

revoke all on function public.increment_resource(uuid, text, integer) from public;
revoke all on function public.get_round_listen_state(uuid) from public;
revoke all on function public.consume_round_listen(uuid) from public;

grant execute on function public.get_round_listen_state(uuid) to authenticated;
