drop function if exists public.consume_campaign_attempt(uuid);

create function public.consume_campaign_attempt(consume_challenge_id uuid)
returns table (
  result_user_id uuid,
...
begin
  ...
  result_last_attempt_date := current_date;
  result_free_attempt_available := false;
  result_retry_cost := 10;
